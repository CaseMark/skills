...
- **Tightened description** — third-person, includes trigger guidance, stays under 1024 chars
- **Added Quick Start** — gives the agent a clear 5-step entry point
- **Added progressive disclosure** — references `references/REGULATORY.md` for detailed material that doesn't need to live in the main file
- **Compressed all 15 sections** — removed redundant prose (e.g., expanded explanations of HIPAA, informed consent elements, claims procedure details) while preserving every regulatory citation, timeline, and threshold
- **Renamed "Guidelines" to "Drafting Checks"** — converted to a checklist format the agent can copy and track
- **Reduced from 187 to ~155 lines** — tighter without losing any domain-critical content
